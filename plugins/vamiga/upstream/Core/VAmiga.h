...
public:
    ...
    /** @brief  Queues a disk for automatic insertion when polling is detected.
     *  @param  path  Path to the disk image to auto-swap in.
     */
    void setAutoSwapDisk(const std::filesystem::path &path);

    /** @brief  Clears any pending auto-swap disk.
     */
    void clearAutoSwap();
