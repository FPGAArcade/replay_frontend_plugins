...
class FloppyDrive final : public Drive, public Inspectable<FloppyDriveInfo> {
    ...
    // Auto-swap: path to next disk, queued by host for sequential disk swapping
    fs::path autoSwapPath;
    bool autoSwapPending = false;
    Cylinder autoSwapMaxCylinder = 0;  // Highest cylinder reached during loading
...
public:
    ...
    //
    // Auto-swap (sequential disk swapping for multi-disk demos)
    //

    // Queues a disk for automatic insertion when the demo requests the next disk
    void setAutoSwapDisk(const fs::path &path);

    // Clears any pending auto-swap disk
    void clearAutoSwap();

private:

    // Checks if conditions are met to perform an auto-swap and executes it
    void checkAutoSwap();
