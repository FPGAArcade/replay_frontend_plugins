...
FloppyDrive::step(isize dir)
{
    ...
    if (didStep) {
        ...
        // Track max cylinder for auto-swap detection
        if (head.cylinder > autoSwapMaxCylinder) {
            autoSwapMaxCylinder = head.cylinder;
        }

        // Auto-swap: when head seeks back to cyl 0 after substantial loading,
        // the demo is requesting the next disk
        if (autoSwapPending && head.cylinder == 0 && autoSwapMaxCylinder >= 20) {
            checkAutoSwap();
        }
...
void
FloppyDrive::setAutoSwapDisk(const fs::path &path)
{
    debug(DSK_DEBUG, "Auto-swap queued on df%td: %s\n", objid, path.c_str());
    autoSwapPath = path;
    autoSwapPending = true;
    autoSwapMaxCylinder = 0;
}

void
FloppyDrive::clearAutoSwap()
{
...
void
FloppyDrive::checkAutoSwap()
{
    auto dmaState = diskController.getState();
    if (dmaState != DriveDmaState::OFF) return;

    debug(DSK_DEBUG, "Auto-swap df%td: maxCyl=%d, swapping to: %s\n",
          objid, (int)autoSwapMaxCylinder, autoSwapPath.c_str());

    swapDisk(autoSwapPath);
    autoSwapPending = false;
    autoSwapPath.clear();
}
