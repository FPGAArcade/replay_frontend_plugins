...
{
    static int eof_log_count = 0;
    int should_log = (eof_log_count < 10);

    if (video_disabled_mode) {
        if (should_log) {
            fprintf(stderr, "[VICE DEBUG] raster_canvas_handle_end_of_frame: early return - video_disabled_mode\n");
            eof_log_count++;
        }
        return;
    }
    ...
    if (vsync_should_skip_frame(raster->canvas)) {
        if (should_log) {
            fprintf(stderr, "[VICE DEBUG] raster_canvas_handle_end_of_frame: early return - vsync_should_skip_frame\n");
            eof_log_count++;
        }
        return;
    }
    ...
    if (!raster->canvas->viewport->update_canvas) {
        if (should_log) {
            fprintf(stderr, "[VICE DEBUG] raster_canvas_handle_end_of_frame: early return - update_canvas=false\n");
            eof_log_count++;
        }
        return;
    }
    ...
    if (should_log) {
        fprintf(stderr, "[VICE DEBUG] raster_canvas_handle_end_of_frame: proceeding to refresh, dont_cache=%d\n",
                raster->dont_cache);
        eof_log_count++;
    }
