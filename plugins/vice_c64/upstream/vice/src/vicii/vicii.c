...
void vicii_change_timing(machine_timing_t *machine_timing, int border_mode)
{
    ...
    /* Update palette BEFORE raster_mode_change(), as it triggers rendering
       which needs the CBM palette to be set up first */
...
{
    log_message(LOG_DEFAULT, "[VICII DEBUG] vicii_init called with flag=%u", flag);
    vicii.fastmode = 0;
    vicii.half_cycles = 0;
...
void vicii_reset(void)
{
    ...
    log_message(vicii.log, "[VICII DEBUG] vicii_reset called");
    ...
    log_message(vicii.log, "[VICII DEBUG] vicii_reset: Setting raster_draw_alarm at clk=%lu draw_cycle=%u",
            (unsigned long)vicii.draw_clk, vicii.draw_cycle);
...
{
    static int alarm_log_count = 0;
    uint8_t prev_sprite_sprite_collisions;
    uint8_t prev_sprite_background_collisions;
    int in_visible_area;

    if (alarm_log_count < 5) {
        log_message(vicii.log, "[VICII DEBUG] raster_draw_alarm_handler[%d]: current_line=%u",
                alarm_log_count, vicii.raster.current_line);
        alarm_log_count++;
    }
