...
/* Exposed for frontend wrappers that need lightweight drive position telemetry. */
unsigned int vice_drive_halftrack[NUM_DISK_UNITS] = {0};
      ...
      unsigned int drive_number, unsigned int drive_base, unsigned int half_track_number, unsigned int disk_side)
{
    if (drive_number < NUM_DISK_UNITS) {
        vice_drive_halftrack[drive_number] = half_track_number;
    }
