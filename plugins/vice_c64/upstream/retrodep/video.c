...
{
   fprintf(stderr, "[VICE DEBUG] video_canvas_create called: index=%d, palette=%p\n",
          canvas->index, (void*)canvas->palette);
   ...
   switch (canvas->index)
   {
      ...
      case 0:
         ...
         fprintf(stderr, "[VICE DEBUG] rendermode set to VIDEO_RENDER_PAL_NTSC_1X1 (%d)\n",
                VIDEO_RENDER_PAL_NTSC_1X1);
         break;
      case 1:
         /* VDC */
   ...
   fprintf(stderr, "[VICE DEBUG] canvas->depth=%d, pix_bytes=%d\n", canvas->depth, pix_bytes);
